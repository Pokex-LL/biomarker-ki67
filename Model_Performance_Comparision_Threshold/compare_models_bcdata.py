...
def load_gt_bcdata_points(img_stem: str, annot_root: Path) -> List[dict]:
    """
    ...
    paths = [
        (annot_root / "positive" / h5_name, 1),
        (annot_root / "negative" / h5_name, 2),
    ]
...
def segment_and_extract(
    ...
    gt_points = load_gt_bcdata_points(img_path.stem, gt_root)
    ...
    for i, pidx in enumerate(pred_idx):
        ...
        if not matched:
            ...
            if pred_cls == match_gt_cls:
                ...
                # equivale a un FP para la clase predicha y FN para la GT
                if pred_cls == "ki67_pos":
                    counts["fp_pos"] += 1
                elif pred_cls == "ki67_neg":
                    counts["fp_neg"] += 1
...
def main():
    ...
    ap.add_argument("--patch_size", type=int, default=50)

    ap.add_argument("--cellpose_model_type", type=str, default="cpsam")
    ap.add_argument("--cellpose_diameter", type=float, default=47)
    ...
    ap.add_argument("--cellpose_min_size", type=int, default=20)
    ...
    # Precompute segmentation + patches once
